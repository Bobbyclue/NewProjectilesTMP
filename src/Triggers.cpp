...
namespace Triggers
{
	...
	{
		return Triggers::init(filename, json_root["Triggers"]);
	}
	...
	namespace Hooks
	{
		...
		public:
			static void Hook()
			{
				...
				_LaunchArrow = trmp.write_call<5>(RELOCATION_ID(17693, 18102).address() + 0xE60, LaunchArrow);

				// 1405504F5 -- MagicCaster::FireProjectileFromSource
				_FireProjectile1 = trmp.write_call<5>(RELOCATION_ID(33670, 34450).address() + 0x575, FireProjectile1);
				// SkyrimSE.exe+5504F5 -- MagicCaster::FireProjectile_0
				_FireProjectile2 = trmp.write_call<5>(RELOCATION_ID(33671, 34451).address() + 0x122, FireProjectile2);

				// 140628dd7 Actor::CombatHit
				_InitializeHitData = trmp.write_call<5>(RELOCATION_ID(37673, 38627).address() + 0x1C6, InitializeHitData);

				// 140628dc8 Actor::CombatHit
				_InitializeHitDataProj = trmp.write_call<5>(RELOCATION_ID(37673, 38627).address() + 0x1B7, InitializeHitDataProj);

				// 1407211ea HitFrameHandler::Handle
				_DoMeleeAttack = trmp.write_call<5>(RELOCATION_ID(41747, 42828).address() + 0x3a, DoMeleeAttack);

				// 14063311e Actor::MagicTarget::AddTarget
				_AddTarget = trmp.write_call<5>(RELOCATION_ID(37832, 38786).address() + 0x16D, AddTarget);

				// 140550a37 MagicCaster::FireProjectile
				_Launch1 = trmp.write_call<5>(RELOCATION_ID(33672, 34452).address() + 0x354, Launch1);

				// 140550a37 TESObjectWEAP::Fire
				_Launch2 = trmp.write_call<5>(RELOCATION_ID(17693, 18102).address() + 0xE60, Launch2);

				// SkyrimSE.exe+754bd8
				_CalcVelocityVector = trmp.write_call<5>(RELOCATION_ID(43030, 44222).address() + 0x78A, CalcVelocityVector);

				// SkyrimSE.exe+74bc21 Proj::Kill // Bugged and will crash the game for fast moving projectiles. Projectile does not always exist by the time function is run?
				//_ClearFollowedObject = trmp.write_call<5>(RELOCATION_ID(42930, 44110).address() + 0x21, ClearFollowedObject);
				...
				_AddImpact1 = trmp.write_call<5>(RELOCATION_ID(42866, 44041).address() + 0xbc, AddImpact1);
				// SkyrimSE.exe+735b06 ConeProj::AddImpact
				_AddImpact2 = trmp.write_call<5>(RELOCATION_ID(42633, 43801).address() + 0x66, AddImpact2);
			}
		...
		private:
			...
			{
				RE::ProjectileHandle* ans = _Launch1(handle, ldata);

				Data data(Data::Type::Spell, ldata);
				if (RE::Projectile* proj = handle->get().get()) {
			...
			{
				if (shandle && shandle->IsValid()) {
					RE::Projectile* proj = (RE::Projectile*)((char*)shandle - 0x4C);  // SE 0x128
